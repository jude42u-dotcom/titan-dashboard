...
# ---------------------------
# AUTO REFRESH (every run)
# ---------------------------
time.sleep(0.1)

# ---------------------------
# LIVE PRICE FUNCTION (FIXED)
# ---------------------------
def get_price(pair):
    try:
...
with tab1:
    ...
    if eurusd_price:
        ...
        st.write(f"Buy Zone: {round(buy_zone, 5)}")
        st.write(f"Sell Zone: {round(sell_zone, 5)}")

        st.write(f"Invalidation: {round(eurusd_price - 0.005, 5)}")
        st.write(f"Target: {round(eurusd_price + 0.005, 5)}")
...
with tab2:
    ...
    if gbpusd_price:
        ...
        st.write(f"Target: {round(gbpusd_price + 0.005, 5)}")
